...
сначала описывается здесь, потом реализуется. JSON, аутентификация — Bearer-токен
(лениво, только для сохранения данных).

## Рецепты
- GET /recipes — список (фильтры: category, country, meal_type, diet, query; пагинация).
- GET /recipes/{id} — деталь.
...
- GET /catalog/ingredients — справочник.
...
- POST /wheel/spin -> { result, links: { recipe_id, scan, restaurants_query } }.
...
- POST /bookings — бронь шефа. Оплата через внешнего провайдера, карты не храним.

## Профиль
- POST /auth/register, POST /auth/login.
- GET /me, GET /me/favorites, POST /me/favorites/{recipe_id}.

Коды ошибок: 400 валидация, 401 нет токена, 404 не найдено, 422 семантика, 429 лимиты.
